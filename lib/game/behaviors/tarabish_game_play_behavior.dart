...
class TarabishGamePlayBehavior extends Behavior<TavernWorld> {
  ...
  void play() {
    parent.tavernBloc.stream.listen((event) {
      switch (event) {
    ...
    //     card.doMove(
    //       parent.playerPiles[j].position,
    //       speed: 15.0,
    //       start: nMovingCards * 0.15,
    //       startPriority: 100 + nMovingCards,
    //       onComplete: () {
    //         parent.playerPiles[j].acquireCard(card);
    //         nMovingCards--;
    //         if (nMovingCards == 0) {
    //           var delayFactor = 0;
    //           for (final tableauPile in parent.playerPiles) {
    //             delayFactor++;
